...
class TagTaTQAReader(object):
    ...
    def _to_instance(self, question: str, table: List[List[str]], paragraphs: List[Dict], answer_from: str,
                     answer_type: str, answer:str, derivation: str, facts:list, answer_mapping: Dict, scale: str, question_id:str, original_uid:str ):
        ...
        if operator_class is None:
            self.skip_count += 1
            return None
        # print("operator class", operator_class, "scale class", scale_class)
        ...
        answer_dict = {"answer_type": answer_type, "answer": answer, "answer_from": answer_from, "original_question_uid": original_uid}
    ...
    def _read(self, file_path: str):
        ...
        for one in tqdm(dataset):
            ...
            for question_answer in questions:
                if question_answer["answer_type"] == "arithmetic":
                    continue
                ...
                try:
                    ...
                    scale = ""
                    original_uid = None
                    if "original_question_uid" in question_answer:
                        original_uid = question_answer["original_question_uid"]
                    instance = self._to_instance(question, table, paragraphs, answer_from,
                                    answer_type, answer, derivation, facts, answer_mapping, scale, question_answer["uid"], original_uid)
        ...
        print("total generated instances", len(instances))
        print("skipping count", total_question_count - len(instances))
        return instances
...
class TagTaTQATestReader(object):
    ...
    def summerize_op(self, derivateion, answer_type, facts, answer, answer_mapping, scale):
        if answer_type == "span":
            if "table" in answer_mapping.keys() and len(answer_mapping["table"]):
    ...
    def _to_test_instance(self, question: str, table: List[List[str]], paragraphs: List[Dict], answer_from: str,
                     answer_type: str, answer:str, scale: str, question_id:str):
        question_text = question.strip()
        dummy_dict = {}
        
        table_cell_tokens, table_ids, table_tags, table_cell_number_value, table_cell_index = \
            table_test_tokenize(table, self.tokenizer, dummy_dict, answer_type)

        paragraph_tokens, paragraph_ids, paragraph_tags, paragraph_word_piece_mask, paragraph_number_mask, \
                paragraph_number_value, paragraph_index, paragraph_mapping_content = \
            paragraph_test_tokenize(question, paragraphs, self.tokenizer, dummy_dict, answer_type)
        question_ids = question_tokenizer(question_text, self.tokenizer)
        ...
        return self._make_instance(input_ids, attention_mask, token_type_ids, paragraph_mask, table_mask,
            paragraph_number_value, table_number_value, paragraph_index, table_index,  paragraph_tokens,
            ...
            for question_answer in questions:
                ...
                try:
                    ...
                    instance = self._to_test_instance(question, table, paragraphs, answer_from,
                                    answer_type, answer, scale, question_answer["uid"])
                    if instance is not None:
                        instances.append(instance)
                except RuntimeError:
                    print(question_answer["uid"])

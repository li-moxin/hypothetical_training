...
class TagopPredictModel():
    ...
    def predict(self, test_data_list):
        ...
        for batch in tqdm(test_data_list):
            current_pred_len = len(pred_json)
            output_dict = self.network.predict(**batch, mode="eval")
            add_len =len(output_dict)
            pred_json.update(output_dict)
            assert len(pred_json) == current_pred_len + add_len
        return pred_json
...
class TagopFineTuningModel():
    ...
    def update(self, tasks, oq_weight = 0.01, hq_weight = 0.01):
        self.network.train()
        output_dict = self.mnetwork(**tasks,oq_weight = oq_weight, hq_weight = hq_weight)
        loss = output_dict["loss"]
